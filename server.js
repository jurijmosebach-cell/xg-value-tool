...
// === API Keys ===
const API_FOOTBALL_KEY = process.env.API_FOOTBALL_KEY; // ⚽ dein richtiger API-Football-Key
...
// === API ROUTE: Ligen an Frontend senden ===
...
function weightedForm(matches, isHome) {
  ...
  matches.slice(-10).forEach((m, i) => {
    const weight = 1 + i * 0.1; // neuere Spiele zählen stärker
...
app.get("/api/games", async (req, res) => {
  try {
    ...
    for (const leagueKey of selected) {
      ...
      // Cache verwenden, wenn jünger als 12h
      ...
      for (const f of fixtures) {
        ...
        // Wahrscheinlichkeit + Value-Berechnung
