...
const LEAGUES = [
  ...
  { key: "soccer_germany_bundesliga", name: "Bundesliga", baseXG: [1.6,1.35] },
  { key: "soccer_germany_2_bundesliga", name: "2. Bundesliga", baseXG: [1.55,1.45] },
  { key: "soccer_spain_la_liga", name: "La Liga", baseXG: [1.45,1.2] },
  { key: "soccer_italy_serie_a", name: "Serie A", baseXG: [1.45,1.25] },
  { key: "soccer_france_ligue_one", name: "Ligue 1", baseXG: [1.55,1.35] },
  { key: "soccer_netherlands_eredivisie", name: "Eredivisie", baseXG: [1.7,1.45] },
  { key: "soccer_sweden_allsvenskan", name: "Allsvenskan", baseXG: [1.55,1.45] },
  { key: "soccer_turkey_super_league", name: "Turkish Süper Lig", baseXG: [1.5,1.35] },
  { key: "soccer_usa_mls", name: "MLS (USA)", baseXG: [1.5,1.3] },
  { key: "soccer_uefa_champs_league", name: "UEFA Champions League", baseXG: [1.55,1.4] },
  { key: "soccer_uefa_europa_conference_league", name: "UEFA Europa Conference League", baseXG: [1.5,1.3] },
  { key: "soccer_uefa_champs_league_qualification", name: "Champions League Qualification", baseXG: [1.5,1.25] }
];
...
function bttsProb(homeXG, awayXG){
  const pNoHome = Math.exp(-homeXG);
  const pNoAway = Math.exp(-awayXG);
  return 1 - pNoHome - pNoAway + (pNoHome*pNoAway);
}
  ...
  for(const league of LEAGUES.filter(l=>leaguesParam.includes(l.key))){
    try {
      ...
      for(const g of data){
        ...
        const probBTTS=bttsProb(homeXG, awayXG);

        const prob={home:probHome, draw:probDraw, away:probAway, over25:probOver25, btts:probBTTS};
        ...
        const value={
          ...
          btts: prob.btts*(odds.over25||1)-1
        };
    ...
    } catch(e){ console.error(`Fehler ${league.name}:`, e.message); }
  }
